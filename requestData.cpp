...
requestData::requestData() :now_read_pos(0), state(STATE_PARSE_URI), h_state(h_start), keep_alive(false), againTimes(0), timer(nullptr)
{
	std::cout << "requestData is constructed" << std::endl;
}

requestData::requestData(int _epollfd, int _fd, std::string _path) : now_read_pos(0), state(STATE_PARSE_URI), h_state(h_start), keep_alive(false), againTimes(0), timer(nullptr), path(_path), fd(_fd), epollfd(_epollfd)
{
	std::cout << "requestData is constructed" << std::endl;
}
...
{
	std::cout << "requestData is destructed" << std::endl;
...
{
	this->timer = timer == nullptr ? mtimer : this->timer;
}
...
void requestData::reset()
{
	...
	this->path.clear();
	this->now_read_pos = 0;
	this->state = STATE_PARSE_URI;
	this->h_state = h_start;
	this->headers.clear();
...
bool timerCmp::operator()(const mytimer * timer1, const mytimer * timer2) const
{
	return timer1->getExpTime() > timer2->getExpTime();
}
